...
Métodos de threshold funcionam alternando entre duas fases. A primeira fase consiste em selecionar um valor limite, chamando de \textbf{threshold}, descrito por $c\star$. Na segunda fase, constrói-se um grafo bipartido com todas as arestas cujo peso é menor ou igual a $c\star$. Em seguida, verifica-se se existe um emparelhamento perfeito nesse grafo. O menor valor de $c\star$ que permite um emparelhamento perfeito é a solução ótima para o problema de atribuição gargalo \cite{burkard}.

Para selecionar o valor limite $c\star$, podemos utilizar uma busca binária entre o menor e o maior peso das arestas do grafo original. A cada iteração, ajustamos o valor de $c\star$ com base na existência ou não de um emparelhamento perfeito no grafo construído. Esse processo continua até que o valor ótimo seja encontrado. Burkard \cite{burkard}, que descreve esse método, utiliza um algoritmo baseado em matrizes para decidir se um emparelhamento é perfeito ou não, porém podemos utilizar qualquer algoritmo clássico de emparelhamento em grafos bipartidos, como os definidos no capítulo \ref{sec:algos_exatos_emp_card_max}, como por exemplo o algoritmo de Hopcroft-Karp.

Lawler \cite{lawler} menciona um método de threshold para solução do problema de atribuição gargalo, porém a definição de threshold dele é levemente diferente, referindo-se ao valor dinâmico do rótulo de um vértice durante busca de caminhos. O algoritmo que ele descreve se assemelha mais aos algoritmos de caminhos aumentantes, que serão discutidos no capítulo \ref{sec:caminhos_aumentantes_min_max}.
...
Assim como os métodos de threshold, o método dual também define um valor limite $c\star$. No entanto, ao invés de 'chutar' um valor usando busca binária, este método define o valor inicial de $c\star$ como sendo o menor valor possível que ele pode assumir. Olhando para a matriz de custos, esse valor é o máximo entre os mínimos de cada linha e cada coluna da matriz:

\begin{equation}
    c\star = \max \left\{ \max_{i} (\min_{j} c_{ik}), \max_{j} (\min_{i} c_{kj}) \right\}
\end{equation}

Com o limite definido, verificamos se é possível encontrar um emparelhamento perfeito no grafo bipartido formado pelas arestas cujo peso é menor ou igual a $c\star$, usando os mesmos algoritmos de antes. Se um emparelhamento perfeito for encontrado, então $c\star$ é a solução ótima. Caso contrário, é necessário aumentar o valor de $c\star$, e para isso nos baseamos no teorema de Kőnig. 

Sabemos que todas as arestas $c_{ij} > c\star$ estão conectadas a um conjunto de linhas $I$ e um conjunto de colunas $J$. Essas linhas e colunas "cobrem" todas as arestas do grafo atual. Para aumentar o valor de $c\star$, precisamos adicionar uma nova aresta que não esteja coberta por esses conjuntos. Procuramos o menor peso entre as arestas não cobertas, ou seja, aquelas que não pertencem às linhas em $I$ nem às colunas em $J$. Esse valor se torna o novo $c\star$:

\begin{equation}
    Novo\space c\star = \min_{i \notin I, j \notin J} c_{ij}
\end{equation}

A partir daqui, o processo se repete. Com o novo valor de $c\star$ e a nova aresta adicionada ao grafo, verificamos novamente a existência de um emparelhamento perfeito. Esse ciclo continua até que um emparelhamento perfeito seja encontrado, momento em que o valor atual de $c\star$ será a solução ótima para o problema de atribuição gargalo.
